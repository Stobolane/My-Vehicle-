...
import com.example.vehiclesystem.Customer;
import com.example.vehiclesystem.DatabaseConnection;
import com.example.vehiclesystem.Vehicle;
import com.example.vehiclesystem.controllers.CustomerDialog;
...
public class EmployeeNewBooking {
    ...
    private void loadAvailableVehicles() {
        vehicleData.clear();
        try (Connection conn = DatabaseConnection.getConnection()) {
            String sql = "SELECT * FROM vehicles WHERE LOWER(status) = 'available' ORDER BY brand, model";
            ResultSet rs = conn.createStatement().executeQuery(sql);
            ...
            while (rs.next()) {
                ...
                System.out.println("Loaded vehicle: " + v.getBrand() + " " + v.getModel() + " - " + v.getStatus());
                vehicleData.add(v);
            }
            ...
            if (vehicleData.isEmpty()) {
                vehicleCombo.setPromptText("No available vehicles");
            } else {
