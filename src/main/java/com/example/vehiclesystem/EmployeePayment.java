...
public class EmployeePayment {
    ...
    private void handleProcessPayment() {
        ...
        // FIXED: Removed "$" symbol to avoid NumberFormatException
    ...
    private void handleConfirmPayment() {
        ...
        String rawInput = amountPaidField.getText().trim(); // Remove spaces
        if (rawInput.isEmpty()) {
            showAlert("Validation Error", "Amount field is empty");
            return;
        }
        ...
        try {
            amountPaid = Double.parseDouble(rawInput);
        } catch (NumberFormatException e) {
            showAlert("Validation Error", "Please enter a valid amount (e.g. 100.00)");
            return;
        }
